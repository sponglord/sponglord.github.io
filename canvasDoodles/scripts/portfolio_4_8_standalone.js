function Portfolio_4_8_s(mouseContext) {
...
//	    _context.fillStyle = "rgba(0,0,0,0.06)";
    ...
    //
    // portfolio_4_8
    // -- 2 dots with segmented line between them
...
//					
    ...
    this.draw = function()
    {
    	var i, img0, img1, dx, dy, dist, max = 400, lineSegs = 10, x, y, j, sx, tx, sy, ty, lsx, lsy;
        ...
        for(i = 0; i < _numImages - 1; i++)
 		{
 			...
 			for(j = i + 1; j < _numImages; j++)
 			{
				...
				if(dist < max)
				{
					_context.strokeStyle = "rgba(255,255,255, 0.5)";
					...
					for(j = 1; j < lineSegs; j += 1) 
					{
						x -= lsx + (Math.random() * 20 - 10);
						y -= lsy + (Math.random() * 20 - 10);
						_context.lineTo(x, y);
				    }
