function Portfolio_2_s(mouseContext){ 
...
//	    _context.fillStyle = "rgba(0,0,0,0.06)";
    ...
    this.init = function()
    {
	    ...
	    // N.B. change the number of pieces e.g. _imageW = 45, _imageH = 40, _numImages = 30 - the also need to change the tiling arrangement: (_images, 10, 3, 25, 200)
...
//					
    ...
    {
    	var i, j, img1, img2, dx, dy, inPlaceCount = 0, easing = 0.08, bounce = -1;
    	...
    	if(!_gotoTarget)
		{
	    	if(img.x + img.radius > _canvasW)
			{
				img.x = _canvasW - img.radius;
				img.vx *= bounce;
			}
			else if(img.x - img.radius < 0)
			{
				img.x = img.radius;
				img.vx *= bounce;
			}
			if(img.y + img.radius > _canvasH)
			{
				img.y = _canvasH - img.radius;
				img.vy *= bounce;
			}
			else if(img.y - img.radius < 0)
			{
				img.y = img.radius;
				img.vy *= bounce;
			}
...
//    	_context.strokeStyle = "rgb(255,255,255)";
         ...
         for(i = 0; i < _numImages; i++) 
         {
             ...
             _context.drawImage(_image, img.column * _imageW, img.row * _imageH, _imageW, _imageH, img.x - _imageW /2 , img.y - _imageH / 2, _imageW, _imageH);
